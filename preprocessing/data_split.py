...
def main(config: DictConfig, logger: MLFlowLogger) -> None:
    ...
    if config.restriction is not None:
        slides = slides[slides[config.restriction.subset_column] == config.restriction.target_value]
    ...
    summary = (
        split.groupby(["set", config.stratify_column])
        .size()
        .reset_index(name="count")
    )
    summary[f"{config.stratify_column}_ratio"] = summary.groupby("set")["count"].transform(
    lambda x: round(x / x.sum(), 3)
)
