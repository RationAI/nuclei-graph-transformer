...
Annotation groups:
    ...
    - "Another pathology": various non-cancerous regions (inflammation, pre-cancer, etc.);
...
This script assumes a metadata CSV files generated by `exploration/save_metadataset` located in the
provided path `<DATASET_NAME>_metadata_uri`.
...
Note: Slides that are labeled positive but have no corresponding annotation file are
      logged in the `missing_annotations.csv`.
"""
