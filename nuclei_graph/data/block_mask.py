...
def mask_mixed_blocks(block_mask: BlockMask, seq_lens: Tensor) -> BlockMask:
    ...
    # If full blocks aren't defined, just apply mask_mod normally
    ...
    for b_seq_len in range(seq_lens.shape[0]):
        num_fully_valid_blocks = seq_lens[b_seq_len] // block_size

        # Query block contains padding -> demote to partial block
        full_kv_idx[b_seq_len, :, num_fully_valid_blocks:] = -1
        # Key/Value block contains padding -> remove from full blocks list
        invalid_kv_mask = full_kv_idx[b_seq_len] >= num_fully_valid_blocks
        full_kv_idx[b_seq_len][invalid_kv_mask] = -1

        # recompute valid full block counts
        full_kv_num[b_seq_len] = (
            (full_kv_idx[b_seq_len] != -1).sum(dim=-1).to(full_kv_num.dtype)
        )
