...
class SpatialPermutationImportanceCallback(Callback):
    ...
    def on_test_batch_end(
        self, trainer: Trainer, pl_module: LightningModule, outputs: Any, batch: Batch, batch_idx: int, dataloader_idx: int = 0
    ) -> None:
    ...
    def on_test_epoch_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        if trainer.sanity_checking or not self.cached_batches: return
        device = pl_module.device

        metric_collection = MetricCollection({
            "AUROC": BinaryAUROC(), "AUPRC": BinaryAveragePrecision(),
            "Accuracy": BinaryAccuracy(), "Precision": BinaryPrecision(),
            "Recall": BinaryRecall(), "Specificity": BinarySpecificity(),
        }).to(device)
        ...
        for batch in tqdm(self.cached_batches, desc="Permuting Spatial Positions", leave=False):
            seq_len = int(batch["seq_lens"].sum().item())
            sup_mask = batch["sup_mask"][:seq_len]
            targets = batch["labels"]["nuclei"][:seq_len][sup_mask].long()
        ...
        metric_drops = {k: base_scores[k].item() - perm_scores[k].item() for k in base_scores.keys()}
        
        self._plot_and_log_results(metric_drops, 
                                   base_prec.cpu(), base_rec.cpu(), 
                                   perm_prec.cpu(), perm_rec.cpu())
    ...
    def _plot_and_log_results(self, metric_drops, b_p, b_r, p_p, p_r) -> None:
        with tempfile.TemporaryDirectory() as output_dir:
            ...
            ax1.bar(list(metric_drops.keys()), list(metric_drops.values()), color="royalblue", edgecolor="black")
            ...
            if (active_run := mlflow.active_run()):
                mlflow.log_artifact(str(out_path / "importance.png"), run_id=active_run.info.run_id)
                mlflow.log_artifact(str(out_path / "pr_curve.png"), run_id=active_run.info.run_id)
