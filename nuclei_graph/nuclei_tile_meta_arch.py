...
class NucleiTileMetaArch(LightningModule):
    ...
    def validation_step(self, batch: LabeledSampleBatch) -> Outputs | None:
        inputs, targets, _ = batch
        logits = self(inputs)
        ...
        seq_len = int(inputs["seq_lens"].sum().item())
        sup_mask = inputs["sup_mask"][:seq_len]

        targets_sup = targets["nuclei"]
    ...
    def test_step(self, batch: LabeledSampleBatch) -> Tensor | None:
        inputs, targets, _ = batch
        logits = self(inputs)
    ...
    def predict_step(
        ...
        inputs, _ = batch
        logits = self(inputs)
        return torch.sigmoid(logits["graph"].view(-1))
