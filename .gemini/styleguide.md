...
**Repository:** `nuclei-graph-transformer` (RationAI)
**Context:** This is a research-focused machine learning repository dedicated to predicting cancer from cell nuclei in prostate WSIs in digital pathology. 

## 🎯 Primary Review Focus
- **Ignore formatting and linting:** We use `ruff` for formatting/linting and `uv` for package management. Assume Action pipelines will catch styling issues. Do not comment on line length, quotes, or basic PEP-8 formatting.
...
- **Repository Structure:**
  ...
  - `nuclei_graph/`: Focus on training loops, PyTorch Lightning modules, and model definitions. Ensure prediction callbacks correctly map predictions to nuclei.
  - `scripts/`: These are job submission templates. Do not review them as strictly as core Python code.
  - `visualization/`: Do not flag memory issues. The rendering at the maximum resolution is intentional.
