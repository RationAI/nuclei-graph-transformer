...
def load_plot_data(inputs_config) -> pd.DataFrame:
    ...
    for item in inputs_config:
        ...
        try:
            ...
            if df.empty:
                print(
                    f"  -> WARNING: [{label}/{dataset}] query '{query}' matched no rows "
                    f"in {uri} — this bar will be missing from the chart",
                    file=sys.stderr,
                )
                continue
            if len(df) > 1:
                print(
                    f"  -> WARNING: [{label}/{dataset}] query '{query}' matched {len(df)} rows "
                    f"in {uri}, expected exactly 1 — taking the first",
                    file=sys.stderr,
                )
...
def create_dataset_grouped_bar_chart(
    ...
    if not x_keys:
        print(
            "No valid groups to plot after applying modality/condition filters.",
            file=sys.stderr,
        )
        return
    ...
    fig, axes = plt.subplots(
        n_rows, n_cols, figsize=figsize, sharey=True, sharex=stacked, squeeze=False
    )
    ...
    for panel_idx, dataset in enumerate(datasets):
        ...
        for m_idx, metric in enumerate(present_metrics):
            ...
            for mod, cond in x_keys:
                row = subset[
                    (subset["Modality"] == mod) & (subset["Condition"] == cond)
                ]
                if not row.empty:
                    row_data = row.iloc[0]
                ...
                if not np.isnan(height):
                    ...
                    ax.annotate(
                        ...
                        xy=(
                            ...
                            hi[j] if not np.isnan(hi[j]) else height,
                        ),
        ...
        ax.set_xticklabels(
            x_labels, fontsize=10, fontweight="bold", rotation=30, ha="right"
        )
...
def main(config: DictConfig, logger: MLFlowLogger) -> None:
    ...
    modality_order = (
        OmegaConf.to_object(modality_order_cfg)
        if OmegaConf.is_config(modality_order_cfg)
        else modality_order_cfg
    )

    condition_order_cfg = config.get("condition_order", [])
    condition_order = (
        OmegaConf.to_object(condition_order_cfg)
        if OmegaConf.is_config(condition_order_cfg)
        else condition_order_cfg
    )
    ...
    with TemporaryDirectory() as output_dir:
        ...
        create_dataset_grouped_bar_chart(
            df, metrics, modality_order, condition_order, out_path
        )
