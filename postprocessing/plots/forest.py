...
def load_plot_data(inputs_config) -> pd.DataFrame:
    ...
    for item in inputs_config:
        ...
        try:
            ...
            if df.empty:
                print(
                    f"  -> WARNING: No rows matched '{query}' in {uri}", file=sys.stderr
                )
                continue
...
def create_faceted_forest_plot(df: pd.DataFrame, metrics: list[str], output_path: Path):
    ...
    fig, axes = plt.subplots(
        n_rows, n_cols, figsize=(8.0 * n_cols, fig_height), sharey=True
    )
    ...
    for row_idx, metric in enumerate(present_metrics):
            ...
            for model in unique_models:
                if model in subset.index:
                    ...
                    point_colors.append(
                        cond_colors.get(
                            row_data["Condition"],
                            fallback_colors[row_idx % len(fallback_colors)],
                        )
                    )
                else:
                    vals.append(np.nan)
        ...
        for col_idx, dataset in enumerate(datasets):
            ...
            if row_idx == 0:
                ax.set_title(
                    f"Dataset: {dataset}", fontsize=14, fontweight="bold", pad=15
                )
                ax.text(
                    1.15,
                    1.02,
